...
class ListCartController {
  async handle(request: Request, response: Response) {
    const { clienteId, active } = request.query;
    const service = new ListCartService();
    try {
      const carts = await service.execute({ clienteId: clienteId as string | undefined, activeOnly: active === undefined ? true : active === 'true' });
